...
Ast_parser::Ast_parser(std::string_view _buffer, Rule_id top_rule) : buffer(_buffer) {
  set_data(root(), Ast_parser_node(top_rule, 0));
  add_track_parent(root());

  level = 0;
  last_added.emplace_back(-1);
}

void Ast_parser::add_track_parent(const hhds::Tree_pos &index) {
  for (int i = last_added.size(); i < index + 1; ++i) {
    last_added.emplace_back(-1);
  }
  I(last_added.size() > (size_t)index);
...
void Ast_parser::up(Rule_id rid_up) {
  ...
  if (down_added == level) {
    ...
    auto &data = get_data(last_added[level]);
    if (!data.rule_id) {
      data.rule_id = rid_up;
    }
    last_added[down_added] = hhds::INVALID;  // Not needed, but helps asserts
  } else if (down_added > level) {
    I((down_added - 1) == level);
    // It has descendents but not current node (add one now)
    I(last_added[level] != hhds::INVALID);
