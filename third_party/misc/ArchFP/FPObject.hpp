...
public:
  ...
  virtual unsigned int outputLgraphLayout(Node_tree& tree, hhds::Tree_pos pos, double startX = 0.0, double startY = 0.0);

  // find a node in tree that can be mapped to this FPObject
  unsigned int findNode(Node_tree& tree, hhds::Tree_pos pos, double calcX, double calcY);
};
